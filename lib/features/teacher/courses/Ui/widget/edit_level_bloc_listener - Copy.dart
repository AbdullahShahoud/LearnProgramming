...
class EidteLevelBlocListener extends StatelessWidget {
  const EidteLevelBlocListener({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocListener<CourserCubitTeacher, CourserCubitTeacherState>(
      listenWhen: (previous, current) =>
          current is LoadingLevelEdit ||
          current is ErrorLevelEdit ||
          current is SuccessLevelEdit,
      listener: (context, state) {
        state.whenOrNull(
          successLevelEdit: (data) {
            alertSuccess(context, data, () {
              context.pushNamed(Routers.home_page_teacher);
            });
          },
          loadinLevelEdit: () => showDialog(
              context: context,
